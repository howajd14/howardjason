...
int bn = nbins;
double hist_tot[bn][bn],bin_center_theta[bn];
double bin_center_phi[bn];
double bin_width_theta = (tmax-tmin)/bn;
double bin_width_phi = (pmax-pmin)/bn;

for(int x =1; x<=bn; x++)
{
	for(int y =1; y<=bn; y++){hist_tot[x-1][y-1]=0;}
...
for(unsigned int x =1; x<=Theta.size(); x++)
{
	...
	hist_tot[b1][b2]=hist_tot[b1][b2]+1; 
}

file hist_OH;
double sz = Theta.size();
for(int y = bn; y>=1; y=y-1)
{
	string holdern; string lineout; lineout = ""; 
	stringstream jh; 
	for(int x=1; x<=bn; x++)
	{
		jh << hist_tot[x-1][y-1]/sz ; jh >> holdern; if(x<bn){lineout = lineout + holdern + "\t"; } if(x==bn){lineout = lineout + holdern;}
		jh.clear();jh.str("");
	} 
	hist_OH.add_line(lineout);
	
}
hist_OH.make_file("2d_hist");
