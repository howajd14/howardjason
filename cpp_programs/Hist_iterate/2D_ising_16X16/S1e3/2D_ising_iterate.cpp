...
int main()
{
  ...
  double  eps = 0.0002; double error = 100; 
file error_out; 
...
while(error>eps)
{
	...
	*/
	if(x%1000==0)
	{
		...
		error = sum/(nbins*1.0); cout << "\t" << error; its = x; 
		stringstream su; su << error; string eoutline; su >> eoutline; 
		error_out.add_line(eoutline); 
		}
	}
}
double Nf = (omega/GE.get_sum()); 
