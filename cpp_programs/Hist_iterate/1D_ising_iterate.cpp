...
int main()
{
  ...
  double J= -0.005; 
  int  n = 10; 
  double omega = 2; for(int x=1; x<=n-1; x++){omega=omega*2;}cout << "\n" << omega << "\n"; 
  double is_a[S][n]; for(int x=1; x<=S; x++){for(int y=1; y<=n; y++){is_a[x-1][y-1]=0;}  }
  srand(time(NULL)); 
  vector<double> ens; 
  for(int x=1; x<=S; x++)
  {
...
for(int x=1; x<=its; x++)
{
	...
	{
		for(int yy=1; yy<=n; yy++){n_a[xx-1][yy-1]=is_a[xx-1][yy-1];}  
	}
	vector<double> ensp;
	vector<double> ensp_oorder; 
	for(int y=1; y<=S; y++)
	{
		int ri = rand()%n + 1; n_a[y-1][ri-1]=n_a[y-1][ri-1]*-1; 
		double E1=0; 
		
		for(int ii=1; ii<=n-1; ii++)
		{
		  E1 = E1 - J*n_a[y-1][ii-1]*n_a[y-1][ii]; 
		}	
	       ensp.push_back(E1); 
	...
	for(int y=1; y<=S; y++)
	{
		...
		if(p <  GEI.get_count(b1)/GEI.get_count(b2))
		{
			...
			for(int ll=1; ll<=n; ll++){is_a[y-1][ll-1]=n_a[y-1][ll-1];}
			ens[y-1]=ensp[y-1]; 	
		}
	}
