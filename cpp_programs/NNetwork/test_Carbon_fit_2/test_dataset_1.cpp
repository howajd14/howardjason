...
int main()
{
   dataset   myds("Dataset_C",14790,49,3); 
   vector<NN>     myNN; 
   srand(time(NULL)); 
   int S = 1; for(int x=1; x<=S; x++){myNN.push_back(NN(1,3,1,52));myNN[x-1].load_NN("CurrentNN");} 
  // cout << "\ngot here\n";
   double del = 0.0001;
   int num_its = 1000000;  
   double val = 0.00001; 
...
for(int x=1; x<=myNN[0].get_nnpl(); x++)
{
	 ...
	 {
		//cout << "\n    " << z  << "\t" << x << "\t" << y;
		int  pm = (rand()%2+1)*2 -3;  // (rand()%RAND_MAX*1.0+1)/RAND_MAX*1.0;
		double vnw = myNN[z-1].get_hW(1,x,y); 
		myNN[z-1].set_hW(1,x,vnw+val*pm, y); 
	 }
}
}
...
for(int x=1; x<=myNN[0].get_non(); x++)
{
	...
	{
		 int  pm = (rand()%2+1)*2 -3;
               double vnw = myNN[z-1].get_oW(x,y); 
		myNN[z-1].set_oW(x,vnw+val*pm,y);
	} 
}
}
