...
int main()
{
   ...
   vector<NN>     myNN; 
   srand(time(NULL)); 
   int S = 1; for(int x=1; x<=S; x++){myNN.push_back(NN(2,3,1,5));} 
   int tnhw=0; 
...
{
	 for(int y=1; y<=myNN[0].get_nhw(ioo); y++)
	 {
		int  pm = (rand()%2+1)*2 -3;  
