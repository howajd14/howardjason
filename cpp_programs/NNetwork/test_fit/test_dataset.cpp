...
int main()
{
   dataset   myds("Dataset",52,49,3); 
   vector<NN>     myNN; 
   srand(time(NULL)); 
   int S = 1000; for(int x=1; x<=S; x++){myNN.push_back(NN(1,4,1,52)); cout << "\n" <<myNN[0].get_nhw(1);} 
  // cout << "\ngot here\n";
   double del = 0.001;
   int num_its = 80000;  
   double val = 0.001; 
...
{
	if(x%1==0){cout << "\n" << x;} 
	if(GE.num_bins()>2000){hist GEP(tempscores); GE = GEP ; max = tempscores[S-1];}
	...
	for(int y=1; y<=S; y++)
	{
		...
		scrh = scrh +  xscore(out,myds,dd); //if(x%100==0&&y==1){cout << "\n" << dd << "\t" << xscore(out,myds,dd);} 
		} 
...
double xscore(const vector<double> calc,  const dataset & mydata, int wds)
{
	double Sc = 0; 
	int x=1; 
	Sc = Sc+ sqrt((calc[x-1]-mydata.get_od(wds,x))*(calc[x-1]-mydata.get_od(wds,x)))/(sqrt(mydata.get_od(wds,x)*mydata.get_od(wds,x)));
	return Sc; 
}
