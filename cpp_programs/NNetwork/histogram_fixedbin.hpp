...
class hist
{
	public: 
		hist();
		hist(const vector<double> & ens, const vector<double> & cnts);
...
hist::hist(const vector<double> & ens, const vector<double> & cnts)
{
	bw = cnts[1]-cnts[0];
	cents = cnts;
	...
	for(unsigned int x=1; x<= ens.size(); x++)
		...
		{
			double lwr = cnts[count-1]-bw/2.0; 
			double upr = cnts[count-1]+bw/2.0; 
