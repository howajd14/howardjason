...
int main()
{
   ...
   vector<NN>     myNN; 
   srand(time(NULL)); 
   int S = 100; for(int x=1; x<=S; x++){myNN.push_back(NN(1,10,1,5));} 
   double hw_del = 0.001;
   double ow_del = 0.001;
...
for(int x=1; x<=num_its; x++)
{
        ...
        for(int xx=1; xx<=S; xx++)
	{
        ...
        for(int y=1; y<=myds.get_nsets(); y++)
        {
                ...
                scores[xx-1]=(scr/(myds.get_nsets()*1.0)); //cout << "\n" << xx << "\t" << scores[xx-1]; 
...
double xscore(const vector<double> calc,  const dataset & mydata, int wds)
{
	double Sc = 0; 
	int x=1; 
	double  mag1 = (calc[x-1]*calc[x-1]/sqrt(calc[x-1]*calc[x-1])) ;
	double  mag2 =	(mydata.get_od(wds,x)*mydata.get_od(wds,x)/sqrt(mydata.get_od(wds,x)*mydata.get_od(wds,x)));
	double diff = mag1 - mag2; 
	Sc = Sc+ (diff*diff/sqrt(diff*diff))/mag2; //cout << "\n" << mag1 << "\t" << mag2; 
