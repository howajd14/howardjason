...
int   get_sign(const vector<double> calc,  const dataset & mydata, int wds);
int main()
{
   dataset   myds("Dataset",458,5,3); 
   vector<NN>     myNN; 
   srand(time(NULL)); 
   int S = 1; for(int x=1; x<=S; x++){myNN.push_back(NN(2,6,1,5));} 
   int tnhw=0; 
...
for(int x=1; x<=S; x++)
{
		...
		vector<double>  out;  myNN[x-1].get_O(out, myds, ods);
		scr = scr + xscore(out,myds,ods);
		int sgn = get_sign(out,myds,ods); 
		cws = cws + sgn; 
...
if(dds%10000==0){cout << "\n" << dds << "\t" << cws/dds << "\t" <<  "\t" << avgS/(dds/10) << "\t" << tempscores[S-1] << "\t" << out[0] << "\t" << myds.get_od(ods,1); }
}
sort(tempscores.begin(),tempscores.end()); 
double max = tempscores[S-1];    
hist GE(tempscores,bins); 
int count =0; 
while(tempscores[S-1]>error_level)
{ 
	count = count+1; 
        if(count%1000==0){max = tempscores[0];}
	...
	for(int y=1; y<=S; y++)
	{
		...
		double scrh=0; 
		vector<double> out; nnP[y-1].get_O(out, myds , ods);
		int sign = get_sign(out,myds,ods); 
		scoresP.push_back(scrh); 
...
int   get_sign(const vector<double> calc,  const dataset & mydata, int wds)
{
	...
	double Sc = 0;
        ...
        double  diff = calc[x-1]-mydata.get_od(wds,x);
        Sc = Sc+ diff*diff/(mydata.get_od(wds,x)*mydata.get_od(wds,x));
        if( calc[x-1]*mydata.get_od(wds,x) < 0 ){sign=-1;}
