...
int main()
{
   dataset   myds("Dataset_xsq",100,1,1); 
   vector<NN>     myNN; 
   srand(time(NULL)); 
   int S = 100; for(int x=1; x<=S; x++){myNN.push_back(NN(1,40,1,1));} 
   double hw_del = 0.001;
   double  bias_del = 0.001; 
   double ow_del = 0.001;
   int num_its = 1000000;  
   double val = 0.001;  
for(int z=1; z<=S; z++)
...
for(int x=1; x<=myNN[0].get_nnpl(); x++)
{
	 for(int y=1; y<=myNN[0].get_nhw(1); y++)
	 {
		int  pm = (rand()%2+1)*2 -3;  
		int rn = rand()%10+1; 
		myNN[z-1].set_hW(1,x,val*pm*1.0*rn, y); 
	 }
}
}
...
double xscore(const vector<double> calc,  const dataset & mydata, int wds)
{
	...
	Sc = Sc+ diff*diff/sqrt(mydata.get_od(wds,x)*mydata.get_od(wds,x));
	//if( calc[x-1]*mydata.get_od(wds,x) < 0 ){Sc=Sc+100;}
	if(!isnan(Sc)&&!isinf(Sc)){
	return Sc*Sc; }else{return 100; }
}
