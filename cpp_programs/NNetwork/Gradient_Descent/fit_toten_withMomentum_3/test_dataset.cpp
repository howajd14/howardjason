...
for(int x=1; x<=num_its; x++)
{
     ...
     if(x>1){
	     NN tempNN = myNN; 
             gradDM(gradNN , myNN , myNNold , del2 , del3);
             myNNold = tempNN; 
     }
     if(x%10000==0)
     {
...
void gradDM( NN & gradNN,   NN & myNN, NN & myNNold,  double p, double p2)
{
         for(int x=1; x<=myNN.get_nlyer(); x++)
         {
                 for(int y=1; y<=myNN.get_nnpl(); y++)
                 {
                         for(int z=1; z<=myNN.get_nhw(x); z++)
                         {
				 double Mterm = (myNN.get_hW(x,y,z) - myNNold.get_hW(x,y,z)); 
                                 myNN.set_hW(x,y,myNN.get_hW(x,y,z)-p*gradNN.get_hW(x,y,z) + p2*Mterm ,z);
          ...
          for(int x=1; x<=myNN.get_nlyer(); x++)
         {
                 for(int y=1; y<=myNN.get_nnpl(); y++)
                 {
				 double Mterm = myNN.get_hnb(x,y) - myNNold.get_hnb(x,y); 
                                 myNN.set_hnb(x,y,myNN.get_hnb(x,y)-p*gradNN.get_hnb(x,y)+p2*Mterm);
           ...
           for(int x=1; x<=myNN.get_non(); x++)
         {
                 for(int y=1; y<=myNN.get_now(); y++)
                 {
				 double Mterm = myNN.get_oW(x,y)-myNNold.get_oW(x,y); 
                                 myNN.set_oW(x,myNN.get_oW(x,y)-p*gradNN.get_oW(x,y)+p2*Mterm,y);
