	...
	for(int i=1; i<=xs; i++)
	{
		for(int j=1; j<=ys; j++)
		{
			for(int k=1; k<=zs; k++)
			{
				...
				//cross_terms = cross_terms + R[index]*(dL_I(i,j,k,1)+dL_I(i,j,k,2)+dL_I(i,j,k,3));
				//cross_terms = cross_terms - (dL_R(i,j,k,1)+dL_R(i,j,k,2)+dL_R(i,j,k,3))*I[index];
