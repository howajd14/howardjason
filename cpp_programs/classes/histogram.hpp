...
class hist
{
	public: 
		...
		unsigned int get_bin(double en) const 
		{
			...
			for(unsigned int x=1; x<=cents.size(); x++)
			{ 
			if(sqrt((cents[x-1]-en)*(cents[x-1]-en))<1e-14){bin=x;}
			}
			return bin; 
		}
...
hist::hist(const vector<double> & ens)
{
	...
	for(unsigned int x = 2; x <= ens.size(); x++)
	{
		...
		if(sqrt((ens[x-1]- en)*(ens[x-1]- en))>1e-14){cents.push_back(ens[x-1]);  en = ens[x-1];  GE.push_back(0); }
	}
	for(unsigned int x=1; x<=cents.size(); x++)
	{
		for(unsigned int y=1; y<=ens.size(); y++)
		{
			if(sqrt((ens[y-1]-cents[x-1])*(ens[y-1]-cents[x-1]))<1e-14){GE[x-1] = GE[x-1]+1; }
		}
	}
...
hist::hist(const hist  & H1,  const hist & H2)
{
	...
	{
		if(sqrt((centso[x-1]-en)*(centso[x-1]-en))>1e-14){cents.push_back(centso[x-1]); en = centso[x-1];  GE.push_back(0);}
	}
	for(unsigned int x =1; x<= cents.size(); x++)
	{
		for(unsigned int y=1; y<= H1.num_bins(); y++)
		{
			if(sqrt((H1.get_en(y)-cents[x-1])*(H1.get_en(y)-cents[x-1]))<1e-14){GE[x-1]=GE[x-1]+H1.get_count(y);}
		}
		for(unsigned int y=1; y<=H2.num_bins(); y++)
		{
			if(sqrt( (H2.get_en(y)-cents[x-1])*(H2.get_en(y)-cents[x-1]) )<1e-14){GE[x-1]=GE[x-1]+H2.get_count(y);}
		}
	}
...
void hist::combine(const hist & H)
{
	...
	{
		if(sqrt((centsp[x-1]-en)*(centsp[x-1]-en))>1e-14){centsnew.push_back(centsp[x-1]); en = centsp[x-1];  GEnew.push_back(0);}
	}
	...
	for(unsigned int x =1; x<= centsnew.size(); x++)
	{
		...
		{
			if(sqrt((cents[y-1]-centsnew[x-1])*(cents[y-1]-centsnew[x-1]))<1e-14){GEnew[x-1]=GEnew[x-1]+GE[y-1];}
		}
		
		for(unsigned int y=1; y<=H.num_bins(); y++)
		{
			if(sqrt((H.get_en(y)-centsnew[x-1])*(H.get_en(y)-centsnew[x-1]))<1e-14){GEnew[x-1]=GEnew[x-1]+H.get_count(y);}
		}
